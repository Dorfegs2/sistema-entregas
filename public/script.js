...
// Busca coordenadas via API do Nominatim OpenStreetMap
...
// Desenha rota usando OpenRouteService (requisição POST)
async function desenharRotaMultiplos(pontos) {
  ...
  return {
    distancia: data.features[0].properties.summary.distance, // metros
    duracao: data.features[0].properties.summary.duration // segundos
  };
}

// Reinsere número no endereço formatado (para não perder a informação)
...
// Resumir o endereço para 4 partes (ex: rua, bairro, cidade, estado)
...
async function calcularRota() {
  ...
  const extras = Array.from(document.querySelectorAll('.endereco-extra')).map(input => input.value.trim()).filter(e => e !== '');
  ...
  try {
    for (const endereco of enderecos) {
      ...
      pontos.push(ponto);
      L.marker([ponto.lat, ponto.lon]).addTo(map).bindPopup(endereco);
    }
...
Solicitante: ${solicitante}`;
  ...
  const numeroWhatsApp = '48988131927'; // Ajuste para seu número
